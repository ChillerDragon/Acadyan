...
public class GuiLevelSelectMP extends Gui
{
	public GuiLevelSelectMP(Game game, int world)
	{
		super(game);
	...
	public GuiLevelSelectMP(Game game)
	{
		this(game, 1);
	}
...
//		add(new GuiLabel(game, Game.ScreenWidth/2, 40, 60, GuiLabel.ALIGNMENT_MIDDLE, Strings.LEVEL_SELECT));
		...
		for(int j = 0; j < rows; j++)
		{
			...
			{
				add(new GuiButton(i + j*cols, x + i*buttonW + i*gap, y + j*buttonH + j*gap, buttonW, buttonH, "Level " +(1 + i + j*cols)));
			}
		}
...
//		if(game.openMap(String.format("%d_%d", world, button.id+1)))
		if(game.openServer(world, button.id+1))
			game.openGui(null);
	}
